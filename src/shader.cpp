...
std::cout << "frag:" << frag << std::endl;
	...
	if (!success) {
        ...
        #ifdef DEBUG
		std::cout<<"ERROR:: COMPILATION OF SHADER FAILED "<<(std::string)vertexShaderPath<<std::endl;
        #endif
	...
	if (!success) {
		...
		#ifdef DEBUG
        std::cout<<("ERROR:: COMPILATION OF SHADER FAILED " + (std::string)fragmentShaderPath)<<std::endl;
        #endif
	}
	...
	if (!success) {
		...
		#ifdef DEBUG
        std::cout<<("ERROR:: LINKING OF PROGRAM FAILED " + (std::string)fragmentShaderPath + " & " + (std::string)vertexShaderPath)<<std::endl;
        #endif
	}
